...
namespace MedPark.Common.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        public IBusSubscriber SubscribeCommand<TCommand>(string @namespace = null, string queueName = null, Func<TCommand, MedParkException, IRejectedEvent> onError = null) where TCommand : ICommand
        {
        ...
        public IBusSubscriber SubscribeEvent<TEvent>(string @namespace = null, string queueName = null, Func<TEvent, MedParkException, IRejectedEvent> onError = null) where TEvent : IEvent
        {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message, CorrelationContext correlationContext, Func<Task> handle, Func<TMessage, MedParkException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            return await retryPolicy.ExecuteAsync<Acknowledgement>(async () =>
            {
                ...
                catch (Exception exception)
                {
                    ...
                    if (exception is MedParkException dShopException && onError != null)
                    {
        ...
        private async Task<Acknowledgement> TryHandleWithRequeuingAsync<TMessage>(TMessage message, CorrelationContext correlationContext, Func<Task> handle, Func<TMessage, MedParkException, IRejectedEvent> onError = null)
        {
            var messageName = message.GetType().Name;
